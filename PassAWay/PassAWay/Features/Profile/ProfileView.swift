        ...
        NavigationStack {
            ZStack {
                ...
                if viewModel.isLoading {
                    ...
                    VStack(spacing: 0) {
                        ScrollView {
                            VStack(spacing: 20) {
                                ...
                                // MARK: My Posts Section (Unchanged)
                                VStack(alignment: .center, spacing: 15) {
                                    Text("My Posts").font(.title3).fontWeight(.heavy).foregroundColor(Color("PassPrimary"))
                                    ...
                                    if viewModel.myItems.isEmpty {
                                        VStack(spacing: 12) {
                                            ...
                                            Text("You haven't listed any items yet.").font(.subheadline).fontWeight(.medium).foregroundColor(Color("PassPrimary").opacity(0.7))
                                        }
                                ...
                                VStack(spacing: 15) {
                                    ...
                                    Button(action: { viewModel.logOut() }) {
                                        HStack {
                                            Image(systemName: "rectangle.portrait.and.arrow.right").foregroundColor(.red).frame(width: 30)
                                            Text("Log Out").fontWeight(.semibold).foregroundColor(.red)
                                            Spacer()
                                        }
                                        .padding(.horizontal).frame(height: 50).background(Color.white).cornerRadius(15)
                                        .overlay(RoundedRectangle(cornerRadius: 15).stroke(Color.red.opacity(0.3), lineWidth: 1))
                                    }
                                }
                                .padding(.horizontal, 25).padding(.top, 10).padding(.bottom, 40)
                            }
                        }
                    }
                    .sheet(isPresented: $showingEditProfile, onDismiss: {
                        // Refresh the profile when the sheet is closed so the new photo/bio shows up instantly!
                    ...
                    // MARK: - Error State (Unchanged)
                    VStack(spacing: 20) {
                        Image(systemName: "exclamationmark.triangle.fill").font(.system(size: 50)).foregroundColor(.red)
                        Text("Failed to load profile.").font(.headline).foregroundColor(Color("PassPrimary"))
                        Text("Your account data might be incomplete or missing from the database.").font(.caption).foregroundColor(.gray).multilineTextAlignment(.center).padding(.horizontal, 40)
                        Button(action: { viewModel.logOut() }) {
                            Text("Force Log Out").fontWeight(.bold).foregroundColor(.white).frame(width: 200, height: 45).background(Color.red).cornerRadius(10)
                        }.padding(.top, 10)
                    }
                }
            }
...
struct ProfileMenuRow: View {
    ...
    var body: some View {
        Button(action: action) {
            HStack {
                Image(systemName: icon).foregroundColor(Color("PassPrimary")).frame(width: 30)
                Text(title).fontWeight(.semibold).foregroundColor(.primary)
                Spacer()
                Image(systemName: "chevron.right").foregroundColor(.gray).font(.caption)
            }
            .padding(.horizontal).frame(height: 50).background(Color("PassLightGreen").opacity(0.5)).cornerRadius(15)
        }
    }
}
